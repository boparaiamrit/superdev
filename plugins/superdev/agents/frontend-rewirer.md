...
- **REFUSE TO REWIRE FAT FILES.** If any file you'd touch in this rewire is > 300 lines, STOP and surface: *"File `<path>` is <N> lines — rewiring it preserves the antipattern. Dispatch `frontend-refactoring` skill on this module first; then re-dispatch me to rewire the decomposed components."* Rewiring a god-file just adds TanStack Query calls to the god-file. The orchestrator should run `frontend-refactoring` BEFORE `frontend-rewirer` per `prototype-to-saas` Phase B.5. See [`frontend-refactoring/SKILL.md`](../skills/frontend-refactoring/SKILL.md).
