...
- `region` — read from `AWS_REGION` at deploy time with a sensible default. Keep the region pinned to where your CockroachDB cluster lives to minimise round-trip latency.
...
| `DATABASE_URL` | `${ssm:/app/DATABASE_URL}` | Full CockroachDB serverless DSN (`postgresql://…:26257/cluster.defaultdb?sslmode=verify-full&sslrootcert=…`). The stock `pgsql` driver reads `url`. See `cockroachdb-serverless-connection.md`. |
| `CACHE_STORE` | `database` | DB-backed cache — Lambda is stateless and Redis would need a VPC. The `cache` table lives in CockroachDB. |
| `SESSION_DRIVER` | `database` | DB-backed sessions for the same reason. The `sessions` table lives in CockroachDB. |
| `QUEUE_CONNECTION` | `sqs` | Jobs dispatch to SQS, not a DB queue (CockroachDB has no `SKIP LOCKED`) and not Redis. |
...
There is **no RDS Proxy** here (that needs a VPC, which we deliberately avoid — see `cockroachdb-serverless-connection.md`). Each warm `web` Lambda holds its own DB connection, so unbounded Lambda concurrency means unbounded connections to CockroachDB serverless. Bound it:
...
Pick a number that keeps peak connections inside the CockroachDB serverless connection budget. Treat it as a launch-time load-test output, not a guess; `deploy-checklist.md` covers verifying it. (`reservedConcurrency` is omitted from the base blueprint above to keep it minimal — add it before any real traffic.)
...
- **Putting the app in a VPC** to "reach the database." CockroachDB serverless is reached over the public internet; a VPC adds NAT cost and ENI cold starts for no benefit here. See `cockroachdb-serverless-connection.md`.
...
- **Unbounded `web` concurrency in production.** Without `reservedConcurrency`, a traffic spike opens unbounded CockroachDB connections (no RDS Proxy without a VPC). Cap it.
